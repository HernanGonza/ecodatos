...
const MemoizedCell = ({ value, columnKey, isUserTable }) => {
  ...
  if (
    isUserTable &&
    (columnKey === "areas" || columnKey === "formularios") &&
    Array.isArray(val)
  ) {
    if (val.length === 0)
      return (
    ...
    return (
      <Group gap={4} wrap="wrap">
        {val.map((item, idx) => (
          <Tooltip
            key={idx}
            label={item.es_editor ? "Editor" : "Lector"}
            withArrow
          >
            <Badge
              variant="light"
              color={item.es_editor ? "cyan" : "blue"}
              size="xs"
              radius="xs"
              leftSection={
                item.es_editor ? (
...
const MemoizedCellWrapper = React.memo(
  MemoizedCell,
  (prev, next) =>
...
const TablaDinamica = forwardRef(({ formulario, onEdit, onView }, ref) => {
    ...
    try {
      ...
      if (isOrphanMode) {
        const { data: users } = await supabase
          .from("perfiles_visibles")
          .select("user_id, nombre");
        setTecnicos(
          users?.map((u) => ({ value: u.user_id, label: u.nombre })) || [],
        );
      }
    } catch (err) {
      notifications.show({
  ...
  const handleBulkReassign = async () => {
    ...
    try {
      ...
      notifications.show({
        title: "Éxito",
        message: `Reasignados ${selectedIds.length} registros`,
        color: "blue",
      });
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (error) throw error;
      setData((prev) =>
        prev.filter((r) => !idsToDelete.includes(String(r.id || r.user_id))),
      );
      setRowSelection({});
      notifications.show({
...
>
  ...
  onView(data.find(r => String(r.id || r.user_id) === row.id))
                    ...
                    >
                      <div
                        ...
                        onMouseDown={(e) => {
                          ...
                          const startHeight = rowHeights[row.id] || ROW_HEIGHT;
                          const onMove = (ev) => {
                            const newH = Math.max(
                              32,
      ...
      <Box className={classes.actionPillContainer}>
        <Transition
          mounted={selectedRowsIds.length > 0 && esEditorDelForm}
          transition="slide-up"
        >
          {(styles) => (
            <Paper
                ...
                <Group gap="xs">
                  ...
                  {isOrphanMode ? (
                    ...
                    <Button
                      ...
                      onClick={() => {
                        setIdsToDelete(selectedRowsIds);
      ...
      {/* Modales */}
      <Modal
        opened={bulkReassignOpen}
        onClose={() => setBulkReassignOpen(false)}
        title={<Text fw={800}>Reasignación Masiva</Text>}
        centered
        radius="md"
      >
        <Stack gap="md">
          <Text size="sm">
            Seleccioná el nuevo responsable para los{" "}
            <b>{selectedRowsIds.length}</b> registros:
          </Text>
          <Select
            label="Nuevo Técnico"
            placeholder="Buscar..."
      ...
      <Modal
        ...
        title={<Text fw={800}>Confirmar eliminación</Text>}
        centered
      >
        <Stack gap="md">
          ...
          <Group justify="flex-end" mt="lg">
            ...
            <Button color="red.6" onClick={confirmDelete}>
              Eliminar
            </Button>
          </Group>
        </Stack>
